#!/usr/bin/env python3
"""siemStartUpParse — unified Setuq SIEM startup + Splunk parse tool.

Merges the project bootstrapper (formerly bootstrap.py) and the Splunk
export/YAML/schema pipeline (formerly splunk_pipeline.py) into one file.

Usage:
    python siemStartUpParse.py up [flags]     # env -> check -> extract -> deps -> run
    python siemStartUpParse.py parse [flags]  # Splunk export -> YAML -> schema_overrides

`up` shells out to `parse` for the extraction phase, so the two halves stay
decoupled at runtime (no shared global state across a single process).
"""

# ==================== PARSE PIPELINE (was splunk_pipeline.py) ====================
"""
...
def phase_extract(refresh: bool, skip_extract: bool, with_field_stats: bool) -> None:
    ...
    if should_skip_extract(SCHEMA_OVERRIDES.exists(), refresh, skip_extract):
        ...
        return
    cmd = [sys.executable, str(SELF), "parse", "--env-file", str(ROOT_ENV)]
    if with_field_stats:
...
def run_bootstrap(argv=None) -> None:
...
def main():
    ...
    if argv and argv[0] == "parse":
        run_pipeline(argv[1:])       # Splunk export / YAML / schema pipeline
    else:
        run_bootstrap(argv)          # bootstrap "up" subcommand (+ flags)
